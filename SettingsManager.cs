    ...
    public class SettingsManager {
        ...
        public void Add(IEnumerable<string> settings) {
            ...
            foreach (var setting in settings) {
                ...
                if (!Settings.Exists(a => splitted.Count() == 2 && a.Alias == splitted.FirstOrDefault())) {
                     System.Console.WriteLine($"Added new setting: {splitted.ElementAtOrDefault(0)} => {splitted.ElementAtOrDefault(1)}");
        ...
        public void Remove(IEnumerable<string> names) {
            ...
            foreach (var name in names) {
                foreach (var setting in Settings.Where(a => a.Alias == name || a.Path == name)) {
                    System.Console.WriteLine($"Removed setting: {setting.Alias} => {setting.Path}");
                }
